...
@router.delete(
    "/finished",
    summary="Deletes all finished root jobs",
)
def delete_finished_jobs(
    user: User = Security(ensure_user),
) -> bool:
    # NOTE: an explicit leading slash — the sibling list route is "s" (no
    # slash) which Starlette appends directly to the "/job" prefix, yielding
    # "/jobs". A slash-less "s/finished" would therefore become "/jobs/finished"
    # instead of the intended "/job/finished".
    # only the local admin may bulk-clear; everyone else keeps their own list
    if user.role != UserRole.ADMIN:
        raise ServerErrors.forbidden
    ctx.jobs.delete_finished()
    return True


@router.delete("/{job_id}", summary="Deletes a finished root job")
def delete_job(
    user: User = Security(ensure_user),
    job_id: str = Path(),
) -> bool:
...
def make_artifacts(
    ...
    if body.volume is not None:
        ...
        volume_id = ctx.volumes.find(body.novel_id, body.volume).id
    crawled = ctx.chapters.list(
        novel_id=body.novel_id,
        volume_id=volume_id,
        is_crawled=True,
    )
    if not crawled:
        raise ServerErrors.no_chapter_content
