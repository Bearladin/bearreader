...
class PyLSP_Session:
    ...
    def _build_cmd(self) -> List[str]:
        if Platform.frozen:
            # Prefer the console build: the windowed app has no stdio to host LSP.
            backend_tool = Path(sys.executable).with_name("backendtool.exe")
            cli_exe = backend_tool if backend_tool.is_file() else Path(sys.executable)
            exe = [str(cli_exe)]
        else:
...
class PythonLanguageServer:
    ...
    def _build_env(self) -> dict:
        ...
        for path in [ctx.config.crawler.user_chinese_sources]:
            if path.exists():
