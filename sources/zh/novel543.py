    ...
    def read_novel_info(self):
        ...
        if author_tag:
            ...
            author_meta_tag = soup.select_one(
                "#detail .media-content p.meta span:-soup-contains('作者：')"
            )
            if author_meta_tag:
    ...
    def download_chapter_body(self, chapter):
        ...
        if next_ and next_.has_attr("href"):
            ...
            if next_url.count("_") > 1:  # if it has multiple _ it means it's a multi-page chapter
                # iterative pagination (no recursion): stop on a missing page
                # or after a sane cap instead of crashing on str + None
                for page in range(2, 11):
                    page_url = next_url.rsplit("_", 1)[0] + f"_{page}.html"
                    page_body = self.download_chapter_body(
                        {"url": page_url, "title": chapter["title"]}
                    )
                    if not page_body:
                        break
                    extracted_content += page_body
