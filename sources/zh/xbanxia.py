...
class Xbanxia(LegacyCrawler):
    ...
    def read_novel_info(self):
        ...
        for div in soup.select("div.book-list ul li"):
            # Recompute the volume id per chapter INSIDE the loop: a div that
            # spans the 100-chapter boundary must start the next volume
            # (old code computed vol_id once per div, mis-placing chapter 101).
            for a in div.select("a"):
                ch_id = len(self.chapters) + 1
                vol_id = (ch_id - 1) // 100 + 1  # chapters 1-100 → vol 1, 101-200 → vol 2
                if ch_id % 100 == 1 or not self.volumes:
                    self.volumes.append(Volume(id=vol_id, name=f"第 {vol_id} 卷"))
